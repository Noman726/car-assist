...
function EditCarPage() {
  ...
  useEffect(() => {
    if (params.id && user) {
      const car = carStorage.getCarById(params.id as string)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Update the car
      const updatedCar = carStorage.updateCar(params.id as string, formData)
  ...
  const handleDelete = async () => {
    ...
    if (confirmDelete) {
      setIsLoading(true)
      try {
        const deleted = carStorage.deleteCar(params.id as string)
        if (deleted) {
          router.push("/dashboard")
        } else {
